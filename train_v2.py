...
from eval_mAP_v2 import evaluate
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_weights", type=str,default="tiny-yolov3_ckpt_epoch-220.pth", help="if specified starts from checkpoint model")
    ...
    # Initiate model
    model = FusionDarknet(opt.model_def, img_size=opt.img_size).to(device)
    ...
    for epoch in range(0, opt.epochs, 1):
        ...
        for batch_i, (_, imgs, figs, points,  targets, image_aug_matrix, lidar_aug_matrix, lss_calib_matrix) in enumerate(tqdm(dataloader,desc="Training")):
            #这里的imgs是BEV特征图,figs是图像, points是点云图列表
            ...
            optimizer.zero_grad()
            loss, outputs = model(imgs, figs, points, image_aug_matrix, lidar_aug_matrix, lss_calib_matrix, targets) #走的是forward函数
            loss.backward()
            optimizer.step()
